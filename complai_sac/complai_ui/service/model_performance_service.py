...
class PerformanceFeatures():
    ...
    def __init__(self, model):
        ...
        #print('target_classes are ',self.config['target_classes'])
        if self.target_classes!=None:
            self.avg_robustness = ["avg_robustness_"+i for i in self.target_classes]
        self.train_data_path = self.model_home+self.config['train_raw_path'].replace(".","",1)
    ...
    def get_train_data(self):
        train_df = pkl.load(open(self.train_data_path, "rb"))
        return train_df
    ...
    def load_datasets(self):
        ...
        if(self.problem_type=='regression'):
            cols_to_drop = ['nearest_synthetic_counterfactual_sparsity', 'distance_values', 'avg_robustness',
                            'explainability_score', 'feature_importance_cf', 'num_features_changes','latest_record_ind']
            ...
            cols_to_drop = ['nearest_synthetic_counterfactual_sparsity', 'distance_values', 'min_robustness', 'min_class_label',
            ...
            cols_to_drop = ['nearest_synthetic_counterfactual_sparsity', 'distance_values', 'min_robustness', 'min_class',
                            'explainability_score', 'num_features_changes', 'feature_importance_cf',
                            'latest_record_ind']
            cols_to_drop.extend(self.avg_robustness)
            val_df = val_df.drop(cols_to_drop, axis=1)
    ...
    def get_numerical_features(self, dataset):
    ...
    def get_feature_performance_distribution(self, val_df, feature):
        val_df['Confusion'] = val_df.apply(self.confusion_func, axis=1)
        return val_df

    def confusion_func(self, row):
        return self._confusion(row, 'ground_truth', 'prediction_label', int(self.prefered_class))
    ...
    def create_slices(self, dataset, train_df, chosen_feature_1=None, chosen_feature_2=None, chosen_bucket_1=None,
        ...
        if ((chosen_feature_1 in categorical_features) and (chosen_feature_2 in categorical_features)):
            ...
            for i in range(len(cats_1)):
                for j in range(len(cats_2)):
                    ...
                    train_sliced_df =  train_df[
                        (train_df[chosen_feature_1] == cats_1[i]) & (train_df[chosen_feature_2] == cats_2[j])]
                    if len(sliced_df) > 0:
                        feature_value_ranges_1.append(cats_1[i])
                        feature_value_ranges_2.append(cats_2[j])
                        num_datapoints.append(len(sliced_df))
                        train_num_datapoints.append(len(train_sliced_df))
            ...
            report_dict['Train Slice Size'] = train_num_datapoints
            ...
            if chosen_bucket_1 == None:
                ...
                for i in range(len(feature_1_tuples)):
                    for j in range(len(cats_2)):
                        ...
                        train_sliced_df = train_df[(train_df[chosen_feature_1] >= feature_1_tuples[i][0]) & (
                                train_df[chosen_feature_1] < feature_1_tuples[i][1]) & (
                                                    train_df[chosen_feature_2] == cats_2[j])]
                        if len(sliced_df) > 0:
            ...
            if chosen_bucket_2 == None:
                ...
                for i in range(len(feature_2_tuples)):
                    for j in range(len(cats_1)):
                        ...
                        train_sliced_df = train_df[(train_df[chosen_feature_2] >= feature_2_tuples[i][0]) & (
                                train_df[chosen_feature_2] < feature_2_tuples[i][1]) & (
                                                    train_df[chosen_feature_1] == cats_1[j])]
                        if len(sliced_df) > 0:
            ...
            if (chosen_bucket_1 == None):
                ...
                for i in range(len(feature_1_tuples)):
                    for j in range(len(feature_2_tuples)):
                        ...
                        train_sliced_df = train_df[(train_df[chosen_feature_1] >= feature_1_tuples[i][0]) & (
                                train_df[chosen_feature_1] < feature_1_tuples[i][1]) & (
                                                    train_df[chosen_feature_2] >= feature_2_tuples[j][0]) & (
                                                    train_df[chosen_feature_2] < feature_2_tuples[j][1])]
                        if len(sliced_df) > 0:
                            feature_value_ranges_1.append(feature_1_tuples[i])
